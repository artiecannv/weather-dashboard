...
function getWeather(event, countryCode = "US") {
  ...
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?zip=${inputBox.value},${countryCode}&appid=${APIkey}`
  )
    .then((response) => {
    ...
    .then((weatherObject) => {
      ...
      let currentTemp = [
        "Current Temperature is:" +
          " " +
          ((weatherObject.main.temp - 273) * 1.8 + 32) +
          "&#8457;",
      ];
      let currentHumid = [
        "Humidity is:" + " " + weatherObject.main.humidity + "%",
      ];
      let currentWind = [weatherObject.wind.speed * 2.237 + "mph"];
  ...
  fetch(
    ...
    .then((weatherObject) => {
      ...
      let hiLow = [
        weatherObject.list[2].main.temp_max,
        weatherObject.list[2].main.temp_min,
      ];
      let humidity = [
        "Humidity is:" + " " + weatherObject.list[4].main.humidity + "%",
      ];
